...
model_parameters_json_file_string = 'twitch_Con_model_exp2.json';%bp
...
%%
% Variables_1s.txt';%
protocol_file_string = 'protocol_exp_P710R2.txt';%_1s';%
model_parameters_json_file_string = 'twitch_P710R_model_exp2.json';%_bp
options_file_string = 'twitch_1_options.json';
...
subplot(1,2,2)
hold on
plot(dist,pops,'color',[1,0.9-0.15*t,0.9-0.15*t],'LineWidth',2);
xlabel('x (nm)')
end
ylabel('bound myosin distribution');
title('P710R')
...
subplot(3,1,1);
hold on
plot(sim_output.time_s,sim_output.muscle_force-baseline_force(2),'r-','LineWidth',2);
xlabel('Time (ms)')
xlim([0 2])
xticks([0 0.5 1 1.5 2])
xticklabels({'-500','0','500','1000','1500'})
...
if plot_experimental
    ...
    plot(t_exp.cell_time_trace+0.5,P710R_F.P710R_c3_force,'r:','LineWidth',3);%+1269.1
       legend('Control model','P710R model','Control exp.','P710R exp.')
    subplot(3,1,3)
    hold on
